...
class QuizApp(QMainWindow):
    ...
    def initUI(self):
        ...
        student_button.clicked.connect(self.show_student_window)
...
class StudentWindow(QWidget):
    ...
    def check_answer(self):
        ...
        correct_answer = self.questions[self.current_question_index][2] if self.current_question_index < len(self.questions) else None

        if correct_answer:
            if answer.strip().lower() == correct_answer.strip().lower():
                self.score += 1
                QMessageBox.information(self, "Правильно!", f"Ваш ответ: {answer} верный!")
            else:
    ...
    def end_quiz(self):
        ...
        QMessageBox.information(self, "Викторина завершена!", f"Ваш результат: {self.score} из {len(self.questions)}.\nВаша оценка: {grade}.")
